...
class ActionManager:
    ...
    def raise_arg_type_mismatch_exception(self, index, lexeme, expected, got, line_num):
        if not self.found_arg_type_mismtach or not self.found_arg_type_mismtach[-1]:
            ...
            raise SemanticException(
                f"#{line_num}: {ARG_TYPE_MISMATCH_SEMANTIC_ERROR.format(index, lexeme, expected, got)}")
    ...
    def pid(self, previous_token, current_token):
        ...
        try:
            ...
            raise SemanticException(f"#{previous_token.lineno}: {SCOPE_SEMANTIC_ERROR.format(previous_token.lexeme)}")
    ...
    def handle_main_function(self, previous_token):
        if previous_token.lexeme == 'main':
            ...
            # Reset data address to ensure variables start at 508
            self.codegen.data_address = DATA_SECTION_START_ADDRESS
        ...
        if self.is_rhs:
            ...
            if symbol.is_function:
                if symbol.symbol_type != INT:
                    raise SemanticException(f"#{current_token.lineno}: {OPERAND_TYPE_MISMATCH_SEMANTIC_ERROR.format(VOID, INT)}")
            else:
                if symbol.is_array:
                    if current_token and current_token.lexeme != "[" and not self.argument_counts:
                        raise SemanticException(f"#{current_token.lineno}: {OPERAND_TYPE_MISMATCH_SEMANTIC_ERROR.format(ARRAY, INT)}")
    ...
    def handle_arg_mismatch(self, current_token, previous_token):
        if len(self.argument_counts) > 0:
            ...
            if index < len(symbol.param_symbols):
                ...
                line_num = previous_token.lineno
                if param_symbol.symbol_type == INT:
                    if current_symbol.symbol_type == ARRAY and (not current_token or current_token.lexeme != "["):
                        self.raise_arg_type_mismatch_exception(index + 1, symbol.lexeme, INT, ARRAY, line_num)
                    if current_symbol.symbol_type == VOID:
                        self.raise_arg_type_mismatch_exception(index + 1, symbol.lexeme, INT, VOID, line_num)
                if param_symbol.symbol_type == ARRAY:
                    if current_symbol.symbol_type == INT:
                        self.raise_arg_type_mismatch_exception(index + 1, symbol.lexeme, ARRAY, INT, line_num)
                    if current_symbol.symbol_type == ARRAY and current_token and current_token.lexeme == "[":
                        self.raise_arg_type_mismatch_exception(index + 1, symbol.lexeme, ARRAY, INT, line_num)
                    if current_symbol.symbol_type == VOID:
                        self.raise_arg_type_mismatch_exception(index + 1, symbol.lexeme, ARRAY, VOID, line_num)
    ...
    def add_break(self, previous_token, current_token):
        if not self.breaks:
            line_num = previous_token.lineno if previous_token else "?"
            raise SemanticException(f"#{line_num}: {BREAK_SEMANTIC_ERROR}")
    ...
    def call(self, previous_token, current_token):
        function_name = self.called_functions[-1]
        arg_count = self.argument_counts[-1]
        
        # Handle output function specially
        if function_name == 'output':
            # Simple print instruction
            arg = self.codegen.semantic_stack.pop()
            self.codegen.push_instruction(Print(arg))
            self.argument_counts.pop()
            self.called_functions.pop()
            return
            
        # For other functions, use the complex mechanism
        ...
        if symbol.param_count != arg_count:
            line_num = current_token.lineno if current_token else "?"
            raise SemanticException(f"#{line_num}: {ARG_COUNT_MISMATCH_SEMANTIC_ERROR.format(function_name)}")
    ...
    def void_check_throw(self, previous_token, current_token):
        if self.void_flag:
            self.void_flag = False
            # Remove the symbol from symbol table
            if hasattr(self.codegen.symbol_table, 'remove_symbol'):
                self.codegen.symbol_table.remove_symbol(self.current_id)
            else:
                # Remove from the last scope
                if self.codegen.symbol_table.scopes and self.codegen.symbol_table.scopes[-1]:
                    self.codegen.symbol_table.scopes[-1].pop()
            line_num = previous_token.lineno if previous_token else "?"
            raise SemanticException(f"#{line_num}: {VOID_SEMANTIC_ERROR.format(self.current_id)}")
    ...
    def check_type(self, previous_token, current_token):
        ...
        if symbol.is_array:
            if current_token and current_token.lexeme != '[' and not self.argument_counts:
                line_num = current_token.lineno if current_token else "?"
                raise SemanticException(f"#{line_num}: {OPERAND_TYPE_MISMATCH_SEMANTIC_ERROR.format(ARRAY, INT)}")
